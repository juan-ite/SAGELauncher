...
def update_db_from_SAGE(uploadToHana):
    table_urls = {'BOM': r'http://10.4.240.65/api/IntegrationAPI/GetBOM',
                  ...
                  'ItemMaster2': r'http://10.4.240.65/api/IntegrationAPI/GetItemMstr',
...
connect_To_HANA = tk.Button(text='Connect to HANA', command=threading.Thread(target=connectToHANA, args=()).start)

connect_To_HANA.pack(pady=20)
...
update_db_from_SAGE_btn.pack(pady=20)
...
run_simulation_btn.pack(pady=20)
...
run_optimization_btn.pack(pady=20)
